...
system("cp $SRCDIR/Makefile $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/mdriver.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/config.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/memlib.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/memlib.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/mm.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/clock.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/clock.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/fcyc.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/fcyc.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/fsecs.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/fsecs.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/ftimer.c $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
system("cp $SRCDIR/ftimer.h $tmpdir") == 0
  or die "ERROR: cp driver files to $tmpdir failed\n";
...
# printf "Style:            / %3.0f\n", $MAXSTYLE;
# print "\n";
printf "Total:       %3.0f / %3.0f\n", $perf_points + $corr_points, $total; 
